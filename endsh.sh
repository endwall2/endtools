...
## Author: Endwall
## Description: Use Torsocks shell inside firejail 
##
...
## License: Endware End User License Agreement version 1.18
#####################################################################
# Dependencies: firejail, torsocks
#####################################################################
# Instructions:  make a directory ~/bin and copy this file there, add this to the $PATH
#                then make the file executable and run it.
# $ mkdir ~/bin
# $ cp endsh.sh ~/bin/endsh
# $ cd ~/bin
# $ chmod u+wrx endsh
...
do
 ...
 if [ "$arg" == "--help" ]
 then
 echo "ENDSHELL:  firejailed torsocks shell for command line"
 echo " "
 echo " USAGE: endsh --option program_name "
 echo " endsh --help    # print usage information"
 echo " endsh --version # print version information"
 shift
 exit 0
 elif [ "$arg" == "--version" ]
 then
 echo "ENDSHELL: version "$version", branch: "$branch", revision date: "$rev_date" "
...
firejail --noprofile --caps.drop=all --netfilter --nonewprivs --nogroups --noroot --seccomp --protocol=unix,inet,inet6 torsocks -i -a "$torsocks_ip" -P "$torsocks_port" --shell "$@" 
