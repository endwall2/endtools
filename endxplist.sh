...
## retrieve the playlist video names
grep "?v=" playlist.html | grep -v "><" |cut -d = -f 5 | cut -d "&" -f 1 >> video_names.tmp

#grep "?v=" playlist.html | cut -d = -f 3  | cut -d \" -f 1 | sort -u >> video_names2.tmp

# concatenate the video names to a new formated list
for vname in $( cat video_names.tmp )
do
