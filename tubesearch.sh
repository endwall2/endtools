...
###################################################
## TITLE: TUBESEARCH
## FILENAME: tubesearch.sh
## TYPE: BOURNE SHELL SCRIPT
## Author: Endwall
## Description: Use Torsocks shell inside firejail with youtube-viewer to search for videos on youtube 
##
## Creation Date: December 18, 2018
## Version 0.02
## Revision Date: December 18, 2018
## Previous Revision Date: December 18, 2018
##
## Copyright: 2018 The Endware Developemnt Team.
## License: The Endware End User License Agreement version 1.18
#####################################################################
# Dependencies: firejail, torsocks, youtube-viewer
#####################################################################
# Instructions:  make a directory ~/bin and copy this file there, add this to the $PATH
#                then make the file executable and run it.
# $ mkdir ~/bin
# $ cp tubesearch.sh ~/bin/tubesearch
# $ cd ~/bin
# $ chmod u+wrx tubesearch
# $ export PATH=~/bin:"$PATH"
#
# Run TUBESEARCH
# $ tubesearch 
# $ tubesearch --order=date --within 1y
####################################################################
#############################################################################################################################################################################
#                                         ACKNOWLEDGMENTS
#############################################################################################################################################################################
...
do
 ...
 if [ "$arg" == "--help" ]
 then
 echo "TUBESEARCH:  search youtube with youtube-viewer in a firejailed torsocks shell"
 echo " "
 echo " USAGE: tubesearch --options "
 echo " $ tubesearch --help  # print usage information"
 echo " $ tubesearch --version   # print version information"
 echo " $ tubesearch --order=date --within=1y   # add youtube-viewer commands"
 echo " Search Command Prompt:"
 echo " :i=2    #to get the information for video #2 etc "
 echo " :next   #scroll to next page of results"
 echo " :h      #get more help commands"
 echo " :q      #quit the program"
 echo " $ youtube-viewer | more  to see more options for commandline inputs"
 shift
 exit 0
 elif [ "$arg" == "--version" ]
 then
 echo "TUBESEARCH: version "$version", branch: "$branch", revision date: "$rev_date" "
 echo "Copyright: 2018, THE ENDWARE DEVELOPMENT TEAM"
 shift
 exit 0
 fi
arghold="$arg"
...
firejail --noprofile --caps.drop=all --netfilter --nonewprivs --nogroups --noroot --seccomp --protocol=unix,inet,inet6 torsocks -i -a "$torsocks_ip" -P "$torsocks_port" youtube-viewer --colorful "$@"
